...
- 記帳功能擴充與修正
  - 新增「總花費」分頁（顯示每位成員本行程總花費）
  - 費用支援自訂分攤金額（每位成員可輸入不同金額）
  - 分攤金額加總檢核（不等於總金額時阻擋送出）
  - 記帳明細新增編輯與單筆還款功能
  - 清算頁支援「以記帳明細為單位」多選批次還款

- 介面一致性優化
  - 記帳相關頭像統一改為成員設定頭像（無頭像時 fallback 預設圖）
  - BottomSheet 圖層與顯示邏輯優化（避免被 sticky header 蓋住）
  - 匯率分頁標籤改為「匯率 / 計算機」雙行顯示

- 機票視覺改版（Schedule）
  - 機票列表卡改為新票券風格（保留漸層票頭）
  - 精簡重複資訊，提升閱讀效率
  - 機場資訊改為「機場代號 / 機場名稱」分行顯示（去程、回程皆適用）

**🔧 技術改進：**

- `Expense` 模組新增 `splitAmounts` 欄位，並維持舊資料相容
- 清算演算法更新：可處理自訂分攤金額與換匯後的四捨五入誤差
- `BottomSheet` 使用 Portal 並統一 z-index 層級
