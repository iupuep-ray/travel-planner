...
### v1.0.5 (2026-03-03) 記帳升級與機票設計優化

**已完成功能：**

✅ **記帳模組強化（Expense）**
- 新增「總花費」分頁：顯示每位成員本行程總花費（資訊揭露）
- 自訂分攤金額：支援每位成員輸入不同分攤額
- 分攤總和檢核：分攤加總必須等於總金額
- 費用明細支援編輯與單筆還款（切換已還款狀態）
- 清算頁支援以「記帳明細」為單位的多選批次還款

✅ **一致性與可用性改善**
- 記帳相關頭像統一使用成員設定頭像（無頭像時 fallback）
- BottomSheet 圖層優化：避免被 sticky header 蓋住
- 匯率分頁標籤調整為雙行顯示（匯率 / 計算機）
...
**技術重點：**
- `Expense` 型別新增 `splitAmounts`（向下相容舊資料）
- `settlement` 計算邏輯支援自訂分攤與換匯誤差修正
- `BottomSheet` 使用 Portal + 統一 z-index 層級管理
